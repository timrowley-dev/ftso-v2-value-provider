...
export class PredictorFeed implements BaseDataFeed {
  ...
  async start() {
    ...
    setInterval(() => {
      ...
      const inactiveExchanges = Array.from(this.exchangeByName.keys()).filter(
        exchange => !activeExchanges.has(exchange)
      );
  ...
  private async watchSingleMarket(exchange: Exchange, marketId: string, exchangeName: string) {
    while (true) {
      try {
        const trades = await retry(async () => exchange.watchTradesForSymbols([marketId], null, 100), RETRY_BACKOFF_MS);
  ...
  private async getFeedPrice(
    ...
    isConversionRate: boolean = false
  ): Promise<number> {
    ...
    this.logger.debug(
      `Price sources distribution for ${feedId.name}: ` + `USDT: ${usdtCount}, USDC: ${usdcCount}, USD: ${usdCount}`
    );
  ...
  private getEnhancedPrice(prices: PriceInfo[]): number {
    ...
    const q1 = this.getQuantile(
      values.sort((a, b) => a - b),
      0.25
    );
    const q3 = this.getQuantile(
      values.sort((a, b) => a - b),
      0.75
    );
    const iqr = q3 - q1;
    ...
    const priceScores = filteredPrices.map(data => {
      ...
      const stabilityWeight = std === 0 ? 1 : Math.exp(-Math.pow(data.price - mean, 2) / (2 * Math.pow(std, 2)));
  ...
  private calculateTrendWeight(current: PriceInfo, allPrices: PriceInfo[], now: number): number {
    ...
    const trend =
      recentPrices.reduce((acc, price, i, arr) => {
        if (i === 0) return 0;
  ...
  private calculateCorrelationWeight(current: PriceInfo, allPrices: PriceInfo[]): number {
    const otherExchangePrices = allPrices.filter(p => p.exchange !== current.exchange).map(p => p.price);
  ...
  private weightedMedian(prices: PriceInfo[]): number {
    ...
    // Remove outliers
    const pricesWithoutOutliers = validPrices.filter(p => p.price >= lowerBound && p.price <= upperBound);
    ...
    weightedPrices.forEach(({ price, weight, exchange, staleness }) => {
      this.logger.debug(
        `${exchange}: $${price.toFixed(4)} | ` + `Weight: ${(weight * 100).toFixed(2)}% | ` + `Staleness: ${staleness}s`
      );
    });
  ...
  private logExchangeSourcesOnce(votingRoundId: number) {
    if (this.lastReportedRound !== votingRoundId) {
      ...
      const exchangeSources = new Map<
        string,
        {
          USDT: {
            configured: number;
            active: number;
            details: Array<{ symbol: string; lastUpdate: number; price: number }>;
          };
          USDC: {
            configured: number;
            active: number;
            ...
            counts.USDT.details.forEach(detail => {
              const status =
                detail.lastUpdate === null
                  ? "No data"
                  : detail.lastUpdate > 300
                    ? `Stale (${Math.floor(detail.lastUpdate)}s old)`
                    : "Active";
